...
import setup_rtd
import paramiko
from ws_multiplexer_reader import *
import shutil
import ftplib
import warnings
    ...
    def main(self):
        ...
        while True:
            ...
            if (curr_time - old_time).total_seconds() / 60 > 5:
                ...
                gps.store_all_csv()
                if ws.s != None: ws.store_all_csv('/home/pi/rtd_global/WS/')
                old_time = curr_time
                ws_c += 5
                ...
                if stats.st_size > 1728000:
                    gps.zip_file()
                if ws.s != None and ws_c > 25:
                    try:
                        ws_name = curr_time.strftime('%y%m%d%H%M')
                        shutil.move('/home/pi/rtd_global/WS/weather_station_MDA_mean.csv',
                                    '/home/pi/rtd_global/WS/ws_uploaded/ws{date}.csv'.format(date=ws_name))
                        Transfer('/home/ec2-user/rtd/vessels/{vess}/ws/data/'.format(
                            vess=setup_rtd.parameters['vessel_name'])).upload(
                            'WS/ws_uploaded/ws{date}.csv'.format(date=ws_name), 'ws{date}.csv'.format(date=ws_name))
                    except:
                        print('Issue recording WS')
                    ws_c = 0
